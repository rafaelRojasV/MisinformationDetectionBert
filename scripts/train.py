...
scripts/train_selective_suppression.py
 ...
 - Selectively suppresses only the 'sm_12 not compatible' user warning.
 - Silences FATAL kernel logs by stderr redirection, but keeps normal warnings for logging.
...
def sm12_warning_filter(record):
    ...
    # If the message mentions "sm_120" or "CUDA capability sm_12" AND "not compatible",
    # we ignore (return False). Otherwise, we allow (return True).
...
logging.captureWarnings(True)       # route warnings through Python's logging
logger = logging.getLogger("py.warnings")
logger.addFilter(sm12_warning_filter)

# ---------------------------------------------------------
# 1) ENV VARS to reduce CUDA logs WITHOUT ignoring all PyTorch warnings
# ---------------------------------------------------------
...
# We'll redirect stderr in order to hide any 'FATAL: kernel' logs that go directly to stderr.
...
try:
    ...
    # You could restore stderr here if you like,
    # but let's keep it suppressed for the entire run.
    pass


# ---------------------------------------------------------
# 2) Normal script starts here
# ---------------------------------------------------------
...
def main():
    if redirect_stderr:
        sys.stderr = io.StringIO()  # continue suppressing any 'FATAL' logs during training
    ...
    logging.info("🔄 Training script started (selective warnings suppression).")
    ...
    # If we want 'metadata' column from domain_prob
    ...
    # If you want to see the suppressed stderr after finishing:
    # if redirect_stderr:
    #     sys.stderr = stderr_backup
    #     # Then print or log what's in the buffer if needed.
